...
export async function uploadAlumniXls(file: File) {
    ...
    const response = await axiosInstance.post("/api/v1/auth/import-alumni", formData, {
        headers: {
...
export async function addAdmin(email: string, password: string) {
    const response = await axiosInstance.post("/api/v1/auth/add-admin", { email, password });
    return response.data;
}
