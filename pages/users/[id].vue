...
<template>
  <LoadingContent :is-loading="isLoading">
    <div v-if="user" class="px-[36px] grid grid-cols-3 gap-x-[36px] gap-y-[54px]">
      <div class="col-span-2">
        <EntityHeader 
          :logo="user.avatar ? `data:image/jpeg;base64,${user.avatar}` : ''"
          :title="`${user.first_name} ${user.last_name}`"
          :subtitle="user.telegram_alias ? `@${user.telegram_alias}` : ''"
        >
          <template #title>
          ...
          <template #subtitle>
            <div class="mt-1 space-y-1">
              <p class="text-sm text-gray-600">{{ formatGraduationYear(user.graduation_year) }}</p>
              <p v-if="user.show_location && user.location" class="text-sm text-gray-600">
                {{ user.location }}
              </p>
              <p 
    ...
    <!-- Location (if shown) -->
    <EntityParagraph v-if="user.show_location && user.location" class="col-span-1">
      <template #title>
        Location
      </template>
      <template #text>
    ...
    <EntityParagraph class="col-span-2">
      ...
      <template #text>
        <div class="rounded-lg border border-gray-200 p-4 text-center">
          <p class="text-gray-500">User's events will be displayed here</p>
        </div>
      </template>
    </EntityParagraph>
    ...
    <EntityParagraph class="col-span-1">
      ...
      <template #text>
        <div class="rounded-lg border border-gray-200 p-4 text-center">
          <p class="text-gray-500">User activity will be displayed here</p>
        </div>
      </template>
    </EntityParagraph>
  </div>
