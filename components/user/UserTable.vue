...
import { CheckCircleIcon, XCircleIcon, UserCircleIcon } from '@heroicons/vue/24/solid'
...
const filteredUsers = computed(() => {
  return props.users.filter(user => {
    ...
    const bannedMatch = filters.banned === null || 
                       (filters.banned ? usersStore.isUserBanned(user.id) : !usersStore.isUserBanned(user.id))
    
    // Filter by verification status
    const verifiedMatch = filters.verified === null || 
...
const handleFilterChange = (type: 'banned' | 'verified', value: string | null) => {
...
<template>
  <div class="w-full">
    <!-- Filters -->
    <div v-if="showFilters" class="flex flex-col sm:flex-row gap-4 mb-6 p-4 bg-gray-50 rounded-lg">
      <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
        <label class="text-sm font-medium text-gray-700 self-center">Status:</label>
        <select 
          :value="filters.verified === null ? 'null' : String(filters.verified)"
          @change="handleFilterChange('verified', ($event.target as HTMLSelectElement).value === 'null' ? null : ($event.target as HTMLSelectElement).value)"
      ...
      <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
        <label class="text-sm font-medium text-gray-700 self-center">Ban Status:</label>
        <select 
          :value="filters.banned === null ? 'null' : String(filters.banned)"
          @change="handleFilterChange('banned', ($event.target as HTMLSelectElement).value === 'null' ? null : ($event.target as HTMLSelectElement).value)"
          class="rounded-md border-gray-300 shadow-sm w-full sm:w-40"
        >
          <option value="null">All</option>
    ...
    <div class="border rounded-lg overflow-hidden">
      <!-- Desktop Headers -->
      <div class="hidden md:grid grid-cols-12 bg-gray-50 p-4 font-medium text-gray-500">
      ...
      <template v-for="user in filteredUsers" :key="user.id">
              ...
              <div class="flex justify-between items-start">
                ...
                <div class="flex items-center gap-1">
                  <button 
                    ...
                    :class="user.is_verified ? 'text-green-600' : 'text-gray-500'"
                    :title="user.is_verified ? 'Verified' : 'Unverified'"
                  >
                    <CheckCircleIcon v-if="user.is_verified" class="w-5 h-5" />
        ...
        <!-- Desktop Row -->
        <div class="hidden md:grid grid-cols-12 items-center p-4 hover:bg-gray-50 border-t">
